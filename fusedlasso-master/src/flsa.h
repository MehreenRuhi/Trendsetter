...
2. The Subgraident Finding Algorithm (SFA) for solving problem (4) (refer to the description of the problem for detail) 
  ...
  int sfa(double *x,     double *gap,
		 ...
		 double *s,     double *g,
		 double tol,    int tau,       int flag, int d, gamma1, gamma2)
  ...
  int sfa_one(double *x,     double *gap,
		 ...
		 double lambda, int nn,       int maxStep,
		 double *s,     double *g,
		 double tol,    int tau, int d, gamma1, gamma2)
...
void R_flsa(double *xnew, double *znew, double *infor, 
            double *lambda1, double *lambda2,
            int *n, int *maxStep, double *tol, 
            int *tau, int *flag, double *x, double *z, int *d, int *gamma1, int *gamma2);
...
void flsa(double *x, double *z, double *infor,
		  double *v, double *z0, 
		  double lambda1, double lambda2, int n, 
		  int maxStep, double tol, int tau, int flag, int d, int gamma1, int gamma2){
	...
	if(d == 1) {//printf("hey, i'm here in IF with d = %d\n", d);                  // MODIFY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	    /*
	    ...
	    */
	    ...
	    Solve the linear system via Thomas's algorithm or Rose's algorithm
            B * z0 = Av   where B = A A^T

            Let B=A A^T. It is easy to get that B is a (n-1) x (n-1) symmetric Toeplitz tridiagonal matrix.
	    ...
	    */
        ...
        Thomas(&zMax, z, Av, nn);
 //       double myzMax = -0.999999;
 //       for(i=0; i < nn; i++) {
 //           if(myzMax < fabs(z[i])) {
 //               myzMax = fabs(z[i]);
 //           }
 //       }
   //     printf("zMax = %lf, nn = %d, myzMax = %lf\n",zMax,n-2,myzMax);
	     /*
	    Rose(&zMax, z, Av, nn);
	    */
    }
    else if(d == 2) {//printf("hey, i'm here in ELSE IF with d = %d\n", d);                   // MODIFY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        /*
	    Compute Av= A*v                  (n=7, nn=5)
			                                             A= [ 1  -2   1   0   0   0   0
												              0   1  -2   1   0   0   0
                                                              0   0   1  -2   1   0   0 
                                                              0   0   0   1  -2   1   0 
            ...
            Let B=A A^T. It is easy to get that B is a (n-d) x (n-d) symmetric Toeplitz banded matrix of bandwith d.
            When n=7, B is defined as (with bandwidth 2):
	    ...
	    */
	    ...
	    /*
	    Solve the linear system via PTRANS-1 algorithm
            B * z0 = Av   where B = A A^T
	    */

        status = PTRANS1(&zMax, z, Av, nn);
        if(status == -1) {
...
//        printf("zMax = %lf, nn = %d, myzMax = %lf\n",zMax,n-2,myzMax);
    ...
    else {//printf("hey, i'm here in ELSE with d = %d\n", d);                   // MODIFY!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        exit(-1);
    }
