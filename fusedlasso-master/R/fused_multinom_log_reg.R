fusedMultinomialLogistic <- function(x, y, lambda, 
                                     ...
                                     class.weights = NULL, opts=NULL, d, gamma1, gamma2) {
  sz <- dim(x)
  n <- sz[1]
  p <- sz[2]
  ...
  if (opts$init == 2) {
    beta <- jitter(array(0, dim = c(p, K)), factor=2)           ########## MODIFIED
    #beta <- array(0, dim = c(p, K))              
    c <- jitter(log(m1 / m2), factor=2)                         ########## MODIFIED
    #c <- log(m1 / m2)                          
  } else {
    if (!is.null(opts$x0)) {
      beta <- opts$x0
...
#      print(paste("MIN(XBETAP) = ",min(xbetap),sep=""))
#      print(paste("MIN(XBETA) = ",min(xbeta),sep=""))
#      print(paste("MIN(BET) = ",min(bet),sep=""))
#      print(paste("MIN(XS) = ",min(xs),sep=""))      

#      print(paste("MAX(XBETAP) = ",max(xbetap),sep=""))
#      print(paste("MAX(XBETA) = ",max(xbeta),sep=""))
#      print(paste("MAX(BET) = ",max(bet),sep="")) 
#      print(paste("MAX(XS) = ",max(xs),sep=""))
      
#      print(paste("MEAN(XBETAP) = ",mean(xbetap),sep=""))
#      print(paste("MEAN(XBETA) = ",mean(xbeta),sep=""))
#      print(paste("MEAN(BET) = ",mean(bet),sep=""))
#      print(paste("MEAN(XS) = ",mean(xs),sep=""))

#      print(paste("MIN(BETA) = ",min(beta),sep=""))
#      print(paste("MAX(BETA) = ",max(beta),sep=""))
#      print(paste("MEAN(BETA) = ",mean(beta),sep=""))

      #aa <- -y.k * (xs + sc)
      aa <- (xs + rep(sc, each = n))
#      print(paste("MIN(AA) = ",min(aa),sep=""))
#      print(paste("MAX(AA) = ",max(aa),sep=""))
#      print(paste("MEAN(AA) = ",mean(aa),sep=""))     
...
#     print(paste("FUN.S = ",fun.s,sep="")) 
  ...
  #      print(paste("ITERSTEP = ", iterStep, sep=""))
  #      print(paste("FUN.BETA = ",fun.beta,sep=""))
  #      print(paste("SUM(ROWSUM(ymat)) = ",sum(rowSums(((y.mat + 1) / 2) * aa)),sep=""))
  #      print(paste("MAX(ROWSUM(ymat)) = ",max(rowSums(((y.mat + 1) / 2) * aa)),sep=""))
  #      print(paste("MIN(ROWSUM(ymat)) = ",min(rowSums(((y.mat + 1) / 2) * aa)),sep=""))
  #      print(paste("SUM(log(ROWSUM(exp(AA)))) = ",sum( log( rowSums(exp(aa)) ) ),sep=""))
  #      print(paste("MAX(log(ROWSUM(exp(AA)))) = ",max( log( rowSums(exp(aa)) ) ),sep=""))
  #      print(paste("MIN(log(ROWSUM(exp(AA)))) = ",min( log( rowSums(exp(aa)) ) ),sep=""))
  #      print(paste("SUM(ROWSUM(exp(AA))) = ",sum( rowSums(exp(aa)) ),sep=""))
  #      print(paste("MAX(ROWSUM(exp(AA))) = ",max( rowSums(exp(aa)) ),sep=""))
  #      print(paste("MIN(ROWSUM(exp(AA))) = ",min( rowSums(exp(aa)) ),sep=""))
        if(is.nan(fun.beta)) {  ########################################### I ADDED THIS CHECK FOR WHEN THE ALGORITHM BEGINS TO BLOW UP
            ...
            if (gr.p > 1) {
              if(d == 1) {
                  if(gamma2 == 1) {
                      ...
                      print("ERROR: gamma2 not equal to 1 or 2")
                  }
              }
              else if(d == 2) {
                  if(gamma2 == 1) {
                      fused.pen <- fused.pen + sum(abs(beta[gr.idx[3:(gr.p)], k] - 2.0*beta[gr.idx[2:(gr.p - 1)], k] + beta[gr.idx[1:(gr.p - 2)], k]))
                  }
                  else if(gamma2 == 2) {
                      fused.pen <- fused.pen + sum((beta[gr.idx[3:(gr.p)], k] - 2.0*beta[gr.idx[2:(gr.p - 1)], k] + beta[gr.idx[1:(gr.p - 2)], k])^2)
                  }
                  else {
                      print("ERROR: gamma2 not equal to 1 or 2")
                  }

  #                print(paste("FUSED.PEN = ", fused.pen, sep=""))
                  
              }
              else {
                  print("ERROR: d not equal to 1 or 2")
              }
   ...
   #     print(paste("PENS = ", pens, sep=""))
        ...
        if(d == 1) {
            ...
            if(gamma2 == 1) {
                pens <- lambda2 * sum(abs(beta[3:p] - 2.0*beta[2:(p-1)] + beta[1:(p-2)]))
            }
            else if(gamma2 == 2) {
                pens <- lambda2 * sum((beta[3:p] - 2.0*beta[2:(p-1)] + beta[1:(p-2)])^2)
            }
            else {
                print("ERROR: gamma2 not equal to 1 or 2")
            }
      ...
      if(gamma1 == 1) {
        ...
        print("ERROR: gamma1 not equal to 1 or 2")
      }
     
 #     print(paste("FUN.BETA = ",fun.beta,sep=""))
 #     print(paste("SUM(ABS(BETA)) = ",sum(abs(beta)),sep=""))  
 #     print(paste("FUNVAL[ITERSTEP] = ",funVal[iterStep],sep=""))
#      print(paste("PENS = ",pens,sep=""))      
